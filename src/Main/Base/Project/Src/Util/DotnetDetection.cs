...
namespace ICSharpCode.SharpDevelop
{
	public static class DotnetDetection
	{
        ...
        public static bool? IsDotnet21CoreInstalled()
        {
            if (GetDotnetCoreRelease() <= 1)
            {
                return true;
            }
            return null; ;
        ...
        static int? GetDotnetCoreRelease()
        {
            using (var key = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Microsoft\WOW6432Node\dotnet\Setup\InstalledVersions\x64\sdk"))
            {
                if (key != null)
                    return key.GetValue("2.1.521") as int?;
            }
            return null;
        }
	}
